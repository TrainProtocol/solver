...
export class PrivateKeyService {
    ...
    private privateKeyConfig!: PrivateKeyConfigService;


    public init(privateKeyConfig: PrivateKeyConfigService): void {
    ...
    public async getAsync(address: string, pkKey: string = this.pkKey): Promise<string> {
        try {
            ...
            const { data } = await this.vault.read(
                `${this.privateKeyConfig.mountPath}/data/${address}`
            );
    ...
    private initLogin(): void {
        this.getTokenAsync =
            this.privateKeyConfig.isK8sAuthEnabled
                ...
                : async () => {
                    await this.vault.userpassLogin({
