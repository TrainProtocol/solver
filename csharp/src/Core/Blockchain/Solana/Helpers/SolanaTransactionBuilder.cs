...
using Train.Solver.Core.Blockchain.Solana.Programs.Models;
...
{
    public static async Task<Result<PrepareTransactionResponse>> BuildHTLCLockTransactionAsync(
       Network network,
       string args)
    {
        ...
        {
            return Result.Fail($"Managed address for {network.Name} is not setup");
        }
        ...
        if (node is null)
        {
            return Result.Fail(new NotFoundError($"Node is not configured on {network.Name} network"));
        }
        ...
        var associatedAccountInstructionResult = await GetOrCreateAssociatedTokenAccount(
            rpcClient,
            builder,
            currency,
        ...
        if (associatedAccountInstructionResult.IsFailed)
        {
            return Result.Fail($"Fail to load wallet in network {network.Name}");
        }
    ...
    public static async Task<Result<PrepareTransactionResponse>> BuildHTLCRedeemTransactionAsync(Network network, string args)
    {
        var request = args.FromArgs<HTLCRedeemTransactionPrepareRequest>();

        if (string.IsNullOrEmpty(request.DestinationAddress))
        {
            return Result.Fail(new NotFoundError($"Receiver address is required"));
        }

        if (string.IsNullOrEmpty(request.SenderAddress))
        {
            return Result.Fail(new NotFoundError($"Sender address is required"));
        }
        ...
        {
            return Result.Fail($"Redeem Descriminator nor configured for network: {network.Name} is not configured");
        }
    ...
    public static async Task<Result<PrepareTransactionResponse>> BuildHTLCRefundTransactionAsync(Network network,
     string args)
    {
        ...
        {
            return Result.Fail($"Refund Descriminator nor configured for network: {network.Name} is not configured");
        }
    ...
    public static async Task<Result<PrepareTransactionResponse>> BuildTransferTransactionAsync(Network network, string args)
    {
        ...
        if (currency is null)
        {
            return Result.Fail(new NotFoundError($"Currency {request.Asset} is not configured"));
        }

        if (request.FromAddress is null)
        {
            return Result.Fail(new NotFoundError("From address is required"));
        }
    ...
    public static async Task<Result<TransactionBuilder>> CreateTransactionInstructionAsync(
       ...
       bool createAssociatedTokenAccount = true)
    {
        ...
        else
        {
            try
            {
                ...
                {
                    destination = AssociatedTokenAccountProgram.DeriveAssociatedTokenAccount(publicKeyToAddress, new PublicKey(currency.TokenContract));
                }
            ...
            {
                return Result.Fail($"Fail to load token wallet: Message {ex.Message}");
            }
        }
    ...
    public async static Task<Result> GetOrCreateAssociatedTokenAccount(
        IRpcClient rpcClient,
