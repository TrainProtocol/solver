...
namespace Train.Solver.Core.Blockchain.Solana.Extensions;
...
public static class SolanaClientExtensions
{
    ...
    public static async Task<Result<SolanaParsedTransactionResponse>> GetParsedTransactionAsync(
        ...
        string transactionId)
    {
        ...
        if (!response.ResponseMessage.IsSuccessStatusCode)
        {
            ...
            return Result.Fail(new NodeError($"Get Transaction with id: {transactionId} fails due to message: {errorMessage}, status code: {response.ResponseMessage.StatusCode}"));
        }
        ...
        {
            return Result.Fail($"Failed to get parsed transaction. TxHash: {transactionId} Status code: {response.StatusCode}");
        }

        if (parsedResult.Result is null)
        {
            return Result.Fail($"Rpc returned empty body. TxHash {transactionId}");
        }

        return Result.Ok(parsedResult);
    }

    public static async Task<Result<SolanaAccountInfoResponse>> GetParsedAccountInfoAsync(
       this IRpcClient client,
       string address)
    {
        var response = await client.NodeAddress
        ...
        if (!response.ResponseMessage.IsSuccessStatusCode)
        {
            ...
            return Result.Fail(new NodeError($"Get account info fails due to message: {errorMessage}, status code: {response.ResponseMessage.StatusCode}"));
        }
    ...
    public static async Task<Result<SolanaBlockEventResponse>> GetParsedEventBlockAsync(
        this IRpcClient client,
        int block)
    {
        try
        {
            ...
            if (!response.ResponseMessage.IsSuccessStatusCode)
            {
                ...
                return Result.Fail(new NodeError($"Get parsed block fails due to message: {errorMessage}, status code: {response.ResponseMessage.StatusCode}"));
            }
        ...
        {
            return Result.Fail($"Failed to get block transactions: {ex.Message}");
        }
    }

    public static async Task<Result<SolanaSignatureStatusResponse>> GetSignatureStatusAsync(
        this IRpcClient client,
        string transactionId)
    {
        var response = await client.NodeAddress
        ...
        if (!response.ResponseMessage.IsSuccessStatusCode)
        {
            ...
            return Result.Fail(new NodeError($"Get transaction status fails due to message: {errorMessage}, status code: {response.ResponseMessage.StatusCode}"));
        }
        ...
        if (parsedResult.Result is null)
        {
            return Result.Fail($"Rpc returned empty body for transaction status check. TxHash {transactionId}");
        }
