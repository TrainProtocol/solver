...
using static Train.Solver.Core.Blockchain.Starknet.Models.GetEventsResponse;

namespace Train.Solver.Core.Blockchain.Starknet.Extensions;
public static class EventDataExtensions
{
    public static Result<StarknetTokenLockedEvent> DeserializeLockEventData(this EventData eventData)
    {
        // Every Uint256 in Starknet consinsts from 2 hex strings
        if (!BigIntegerExtensions.TryParse(eventData.Keys[1], eventData.Keys[2], out var id))
        {
            return Result.Fail($"Failed to deserialize commitId in lock event: txHash {eventData.TransactionHash}");
        }
        ...
        {
            return Result.Fail($"Failed to deserialize timelock in lock event: txHash {eventData.TransactionHash}");
        }
    ...
    public static Result<StarknetTokenCommittedEvent> DeserializeCommitEventData(this EventData eventData)
    {
        var cursor = 0;

        if (!BigIntegerExtensions.TryParse(eventData.Keys[1], eventData.Keys[2], out var id))
        {
            return Result.Fail($"Failed to deserialize commitId in commit event: txHash {eventData.TransactionHash}");
        }
        ...
        {
            return Result.Fail($"Failed to deserialize amount in commit event: txHash {eventData.TransactionHash}");
        }
        cursor += 2;

        if (!BigIntegerExtensions.TryParse(eventData.Data[cursor], eventData.Data[cursor + 1], out var timelock))
        {
            return Result.Fail($"Failed to deserialize timelock in commit event: txHash {eventData.TransactionHash}");
        }
        cursor += 2;
