...
public class SwapWorkflow
{
    ...
    public async Task RunAsync(HTLCCommitEventMessage message)
    {
        ...
        var hashlock = await ExecuteLocalActivityAsync(
            (SwapActivities x) =>
                ...
                new()
                {
                    StartToCloseTimeout = TemporalHelper.DefaultActivityOptions(Constants.CSharpTaskQueue).StartToCloseTimeout,
                    ScheduleToCloseTimeout = TemporalHelper.DefaultActivityOptions(Constants.CSharpTaskQueue).ScheduleToCloseTimeout,
                });
        ...
        try
        {
            ...
            if (_htlcAddLockSigMessage != null)
            {
                ...
                var childWorkflowTask = ExecuteChildWorkflowAsync(TemporalHelper.ResolveProcessor(_htlcCommitMessage.SourceNetwrokGroup), [new TransactionContext()
                {
                    ...
                    SwapId = _swapId
                }], new() { Id = TransactionProcessorBase.BuildId(_htlcCommitMessage.SourceNetwork, TransactionType.HTLCAddLockSig) });
            ...
            var redeemInSourceTask = ExecuteChildWorkflowAsync(TemporalHelper.ResolveProcessor(_htlcCommitMessage.SourceNetwrokGroup), [new TransactionContext()
            {
                ...
                FromAddress = _solverManagedAccountInSource!,
                SwapId = _swapId
            }], new() { Id = TransactionProcessorBase.BuildId(_htlcCommitMessage.SourceNetwork, TransactionType.HTLCRedeem) });
    ...
    [WorkflowUpdate]
    public async Task<bool> SetAddLockSigAsync(string networkName, AddLockSignatureRequest addLockSig)
    {
        try
        {
